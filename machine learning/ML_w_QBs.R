...
offensive_epa_data <- ML_data %>%
  ...
  group_by(game_id, season, week, posteam, home_team, home_qb_name) %>%
...
defensive_epa_data <- ML_data %>%
  filter(!is.na(epa), !is.na(posteam), play_type == "pass" | play_type == "run" ) %>%
  group_by(game_id, season, week, defteam, away_team, away_qb_name) %>%
...
epa_data <- left_join(offensive_epa_data, defensive_epa_data, by = c("game_id", "season", "week", "posteam" = "defteam")) %>%
  ...
  select(game_id, season, week, home_team, away_team, posteam, defteam, home_qb_name, away_qb_name,
...
epa_data_qbs <- full_join(spread_data, epa_data, by = "game_id")
epa_data_qbs <- epa_data_full %>% distinct() 


write.csv(epa_data_qbs,'/Users/tcjurgens/Documents/Data_Final_Project/NFL-Analytics-Project/machine learning/ML_epa_qbs_data.csv')
